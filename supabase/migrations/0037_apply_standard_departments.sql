-- Apply standard departments DEPT-001 … DEPT-009 (callable from app via RPC).
...
BEGIN
  ...
  FOR i IN 1 .. array_length(names, 1) LOOP
    ...
    IF v_id IS NOT NULL THEN
      ...
      WHERE id = v_id
        AND (code IS DISTINCT FROM v_code OR name IS DISTINCT FROM names[i]);
      IF FOUND THEN
...
GRANT EXECUTE ON FUNCTION public.apply_standard_departments() TO authenticated;

COMMENT ON FUNCTION public.apply_standard_departments IS
  'Upsert 9 standard departments with codes DEPT-001 … DEPT-009 for current user company';
