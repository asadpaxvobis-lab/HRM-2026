...
DROP POLICY IF EXISTS zkt_fetch_runs_select ON public.zkt_device_fetch_runs;
CREATE POLICY zkt_fetch_runs_select ON public.zkt_device_fetch_runs
  FOR SELECT TO authenticated
  USING (company_id = public.current_user_company_id());

DROP POLICY IF EXISTS zkt_fetch_entries_select ON public.zkt_fetch_log_entries;
